...
* MUST validate `expiry_height` against its accepted range: no lower than
  ...
  bound is mandatory; a board request does not grant a client discretion to
  choose an arbitrarily long expiry. Before expiry, the protocol guarantees the server no
  ...
  leave the board unresolved for an arbitrarily long time. The
  principal is user-funded, so this is primarily a bound on unresolved
  protocol state and server recourse.
