...
- **PV-3 MUST\*** — The taproot carries **two** leaves: `timelock-sign(expiry_height, S)` (server post-expiry sweep) and a constant unspendable **domain-marker** (`OP_RETURN <C>`) that distinguishes the output key from a board funding output's (distinct VTXO type → distinct, non-colliding construction; MR-1 note §2d). The `pubkey` policy's `delayed-sign(exit_delta, A)` unilateral-exit leaf is **absent** — there is no user VTXO-level exit leaf. — `02-vtxo.md:168-178`; `08-channels.md:176-186` *(AMENDED 2026-08-03: was "exactly one leaf"; the marker leaf was added for domain separation.)*
