...
- **PV-1 MUST\*** — A channel VTXO's output uses policy `channel-funding` (type byte `0x0c`), fields: `user_pubkey` (33 bytes) only. — `02-vtxo.md:149-156`; `08-channels.md:164-166`
- **PV-2 MUST\*** — Internal key = `musig(user_pubkey, server_pubkey)`; this is both the bridge's spend path and the path used to forfeit/refresh/cooperatively-spend the VTXO. — `02-vtxo.md:158-171`; `08-channels.md:167-174` *(AMENDED 2026-08-03: `channel-funding` is **domain-separated** from a board funding output — it is **not** the bare `cosign taproot`; see PV-3 and the MR-1 note §2d.)*
- **PV-3 MUST\*** — The taproot carries **two** leaves: `timelock-sign(expiry_height, S)` (server post-expiry sweep) and a constant unspendable **domain-marker** (`OP_RETURN <C>` at leaf version `0xc0`, `C = tagged_hash("ark/channel-funding-domain-v1", "") = f1133dc9…5ea609` — exact bytes pinned in `02-vtxo.md`) that distinguishes the output key from a board funding output's (distinct VTXO type → distinct, non-colliding construction; MR-1 note §2d). The `pubkey` policy's `delayed-sign(exit_delta, A)` unilateral-exit leaf is **absent** — there is no user VTXO-level exit leaf. — `02-vtxo.md:168-178`; `08-channels.md:176-186` *(AMENDED 2026-08-03: was "exactly one leaf"; the marker leaf was added for domain separation.)*
...
- **PV-7 MUST\*** — A `channel-funding` VTXO (`0x0c`) is rejected by any decoder that predates it (unknown policy-type bytes are rejected) — channel VTXOs are only meaningful between channel-aware parties. — `08-channels.md:1839-1841`
...
- **I-10** *(added at G0 — codex F3)* — OP-21's suspend-until-reestablished is **unachievable on stock LDK**: withdrawing the funding confirmation force-closes an active channel rather than suspending it. **Stage-1 disposition**: the chain-feed adapter withdraws the virtual funding confirmation only on a genuine (deep) reorg of the anchor — anchors are typically deeply confirmed at open — and the resulting stock force-close is accepted as the fail-closed direction the spec itself prefers (WD-15). Proven in the spike. Related stock-LDK constraint, same origin — the synthesized confirmation POSITION (**this was a spec gap when written; it LANDED 2026-07-30 in the virtual-funding trust bullet of `08-channels.md`**). LDK derives a real SCID from the fed `(height, tx_index, vout)` and hard-asserts on per-node collisions; the height is pinned by OP-18 (the anchor's actual height) but the transaction index is pure fiction — it cannot reflect any on-chain reality. Requirements the spec must state (virtual-funding section):
  ...
  (iv) peers need NOT agree on the position — the real SCID is node-local bookkeeping for these unannounced channels (aliases carry every wire use), and the channel MUST NOT be announced (an announcement would sign a fictitious SCID);
  recommended scheme: derive the index from the bridge txid (24 bits, floored to ≥1) with a persisted per-node collision bump — stability for free, incidental peer agreement.
  **Old-branch evidence**: the client derived exactly this (txid low 3 bytes, `.max(1)`, with a FIXME demanding a production allocation scheme — `bark-lightning/src/client_node.rs:1571-1586`), but captaind hardcoded **position 1 for every channel** (`bark-lightning/src/virtual_funding.rs:534-536`) — a latent duplicate-SCID assert/panic for any two channels whose anchors share a block height, never hit in e2e only because anchors happened to land at distinct heights. The two sides therefore never agreed on the real SCID and everything worked — empirical confirmation of (iv).
